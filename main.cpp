...
void stitchPrototype(const std::string& directory_path, const std::string&  file_mask)
{
    vector<Mat> images;
    vector<String> image_paths;

    // Use the glob function to get all files in the directory matching a specific pattern (e.g., "*.jpg")
    glob(file_mask, image_paths);
    ...
    if (status != Stitcher::OK) {
        static std::map<Stitcher::Status, std::string> error_messages = {
                {Stitcher::OK, "OK"},
...
int main(int argc, char* argv[]) 
{
    ...
    stitchPrototype(directory_path, file_mask);
